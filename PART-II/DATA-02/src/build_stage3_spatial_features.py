...
def read_seoul_points() -> tuple[pd.DataFrame, dict]:
    ...
    with zipfile.ZipFile(SMALL_ZIP_PATH) as archive:
        ...
        for info in archive.infolist():
            ...
            if "서울" in decoded_name:
                members.append((info, decoded_name))

        print(
            "[확인] 서울 후보:",
            [decoded_name for _, decoded_name in members],
        )

        if len(members) != 1:
            raise SystemExit(
                f"서울 CSV 판별 실패: 후보 {len(members)}개"
            )

        member_info, member = members[0]
        print(f"[확인] 서울 원본 선택: {member}")

        with archive.open(member_info) as stream:
            raw = pd.read_csv(
                stream,
                encoding="utf-8-sig",
                dtype={
