...
def build_portfolio_card(portfolio, top_score=None):
    """
    추천 포트폴리오 카드
    ...
    is_primary = portfolio.get("is_top_recommendation", rank == 1)
    gap_tier = portfolio.get("gap_tier")
    ...
    # risk_factors는 항상 최소 1개(없으면 "특별히 확인된 위험 요인 없음")가
    # 채워져 있으므로, 그게 아닌 실제 위험 문장이 있는지로 판단한다.
    has_real_risk_text = bool(risk_items) and risk_items[0] != "특별히 확인된 위험 요인 없음"

    if overlap_risk is not None and (overlap_risk > 0 or has_real_risk_text):
        value_children = [
            html.Span(
        ...
        if has_real_risk_text:
            value_children.append(
                html.Span(
                    f" · {risk_items[0]}",
                    className="portfolio-risk-value-detail",
                )
            )
        risk_block = html.Div(
            [
                ...
                html.Span(value_children),
            ],
            className="portfolio-risk",
        )

    elif has_real_risk_text:
        risk_block = html.Div(
            [
                html.Span(
                    "주의",
...
def build_portfolio_section(
    ...
    intro = html.Div(
        [
            ...
            html.Div(
                ...
                "점수 차이가 매우 근소한 경우 1·2위를 함께 최우선 추천으로 표시하며, "
                "1위와 근소한 차이의 대안은 별도로 표시됩니다.",
                className="portfolio-page-description",
            ),
            html.Div(
                "해당 점수는 선택하신 출점 점포 수 내의 결과입니다. "
                "현재 옵션의 순위에서 점수를 비교하는 것을 권장드립니다.",
                className="portfolio-page-note",
            ),
        ],
    ...
    # ---------------------------------------------------------
    card_columns = []
    top_score = filtered[0]["portfolio_score"] if filtered else None

    for p in filtered:
        card_columns.append(
            dbc.Col(
                build_portfolio_card(p, top_score=top_score),
