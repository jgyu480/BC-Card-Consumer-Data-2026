...
def build_dna_summary(brand, desired_store_count=None):
    ...
    stat_items = []
    if dna.get("evidence_store_count"):
        stat_items.append(("기존 매장 데이터", f"{dna['evidence_store_count']}곳"))

    if recommendations:
        district_count = len({
            r.get("district_nm") for r in recommendations if r.get("district_nm")
        })
        if district_count:
            stat_items.append(("추천 상권 분포", f"서울 {district_count}개 자치구"))
        ...
        left_children.append(
            html.Div(
                [
                    html.Div(
                        [
                            html.Div(label, className="overview-mini-stat-label"),
                            html.Div(value, className="overview-mini-stat-value"),
                        ],
                        className="overview-mini-stat-tile",
                    )
                    for label, value in stat_items
                ],
                className="overview-mini-stat-group",
            )
        )
    ...
    if top_area:
        ...
        gauge_card_body = [
            html.Div(
                "단일 후보 적합도",
                className="overview-visual-title overview-title-has-tooltip",
                title=(
                    "이 브랜드가 지금까지 입점해온 소비환경 적합도, 브랜드 상대적 강점, "
                    "시장 규모, 경쟁 우위와 상권 데이터의 안정성 등을 종합적으로 고려하여 "
                    "산출된 최종 점수입니다. 예상 매출, 생존 확률, 투자수익률을 의미하지 "
                    "않으며, 브랜드의 기존 운영 환경과 상권 특성 간의 적합도를 나타내는 "
                    "의사결정 참고 지표입니다."
                ),
            ),
            _suitability_gauge(
                top_area["suitability_score"],
                top_area.get("recommendation_label"),
            ),
        ]
