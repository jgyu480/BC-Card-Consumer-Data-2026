"""
최종 조립 스크립트 (assemble_result_v2.py)
=====================================================
...
RECOMMENDATIONS_PATH = APP03_DIR / "outputs" / "recommendations_output_v2.json"
PORTFOLIOS_PATH = ROOT / "PART-VI" / "PORTFOLIO-01" / "outputs" / "portfolios_output_v2.json"
LLM01_DIR = ROOT / "PART-V" / "LLM-01" / "scripts"

OUTPUT_PATH = APP03_DIR / "outputs" / "result_v2.json"
...
UI_COPY = {
    ...
    # 출점 수(1개/2개/3개) 선택 UI 근처에 배치할 안내 문구.
    # portfolio_score는 desired_store_count별로 각자 정규화돼 있어서
    # (Phase 7 참고) 서로 다른 출점 수끼리 점수를 직접 비교하면 안 됨.
...
def load_json(path):
    if not path.exists():
        raise SystemExit(
            f"[에러] {path} 가 없습니다. "
            f"build_recommendations_v2.py / build_portfolios_v2.py를 먼저 실행하세요."
        )
...
# =========================================================
# 2. brand_type — v2에서는 build_recommendations_v2.py가 이미 계산해서
...
def assemble_brand(brand_id, rec_data, portfolio_data):
    ...
    return {
        ...
        "brand_type": rec_data["brand_type"],  # build_recommendations_v2.py가 이미 계산
        "ai_explanation": rec_data.get("ai_explanation"),  # B(LLM-01) 문단, top3 밖이면 None
...
def main():
    ...
    for brand_id, rec_data in rec_all.items():
        ...
        if not rec_data.get("ai_explanation"):
            missing_explanation_brands.append(rec_data["brand_name"])
    ...
    if missing_explanation_brands:
        print(f"[참고] AI 설명(ai_explanation)이 없는 브랜드: {missing_explanation_brands}")
